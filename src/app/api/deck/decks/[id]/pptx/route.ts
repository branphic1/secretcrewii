...
import { buildDeckPptx, planSlides } from "@/lib/deck/pptxBuilder";
...
export async function GET(_req: Request, { params }: { params: { id: string } }) {
  ...
  const slides = planSlides(deck as Deck, profile as DeckProfile | null);
  const buf = await buildDeckPptx(deck as Deck, profile as DeckProfile | null, slides);

  const safeTitle = (deck.title || "deck").replace(/[\\/:*?"<>|]/g, "_").slice(0, 80);
  const dateStr = new Date(deck.meeting_date).toISOString().slice(0, 10);
  const fileName = encodeURIComponent(`${dateStr}_${safeTitle}.pptx`);
