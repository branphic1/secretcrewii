...
export default function PlanVsActual({ entry, categories }) {
  const rows = dayGap(entry);
  if (rows.length === 0) return null;

  const maxAbs = Math.max(...rows.map((r) => Math.max(r.planned, r.actual)), 1);

  return (
    <Section title="계획 vs 실제" subtitle="카테고리별 시간 배분 차이">
            ...
            <div key={r.categoryId} className="flex items-center gap-3">
              ...
              <div className="flex-1 flex flex-col gap-0.5">
                <div className="flex items-center gap-2">
                  ...
                  <span className="display italic text-xs tabular-nums" style={{ color: '#78716C', minWidth: '2.4rem', textAlign: 'right' }}>
              ...
              <span className="display italic text-sm tabular-nums" style={{ color: gapColor, minWidth: '3rem', textAlign: 'right' }}>
                {gapLabel}h
              </span>
            </div>
          );
